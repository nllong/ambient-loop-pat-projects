...
class SetWindowToWallRatio < OpenStudio::Ruleset::ModelUserScript
  ...
  def description
    return "Set the window to wall ratio for all facades to a specific value."
  end
  ...
  def run(model, runner, user_arguments)
    ...
    # loop through surfaces finding exterior walls
    ...
    runner.registerInitialCondition("The model's initial window to wall ratio for the exterior walls was #{starting_wwr}.")

    if not exterior_walls
      runner.registerAsNotApplicable("The model has no exterior walls and was not altered.")
      return true
    elsif not windows_added
      runner.registerAsNotApplicable("The model has exterior walls, but no windows could be added with the requested window to wall ratio")
      return true
    end
    ...
    runner.registerFinalCondition("The model's final window to wall ratio for the exterior walls is #{final_wwr}. Window area increased by #{neat_numbers(increase_window_area_ip.value,0)} (ft^2). The material and construction costs increased by $#{neat_numbers(envelope_cost,0)}.")
