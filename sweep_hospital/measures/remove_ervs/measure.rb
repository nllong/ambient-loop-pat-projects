...
# start the measure
class RemoveERVs < OpenStudio::Measure::ModelMeasure
  # human readable name
  def name
  ...
  def description
    return 'This measure iteratures through all airloops in the model, and removes any HeatExchanger:AirtoAir:SensibleandLatent object that is connected
  ...
  def run(model, runner, user_arguments)
	...
	#Start body of measure 
	model.getAirLoopHVACs.each do |airloop|
	  oasys=airloop.airLoopHVACOutdoorAirSystem.get
              oasys.oaComponents.each do |comp|
              if comp.to_HeatExchangerAirToAirSensibleAndLatent.is_initialized
              puts comp
              comp.remove()
              end 
			  end 
...
RemoveERVs.new.registerWithApplication
