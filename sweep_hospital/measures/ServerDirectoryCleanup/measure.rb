    ...
    file_types.each do |k,v|
      ...
      if v == "Sizing Run Directories"
        ...
        Dir.glob("./../**/SizingRun").select{ |e| File.directory? e }.each do |f|
         ...
         ##AA added the chunk below to get rid of prototype measure directories 12/20 
         Dir.glob("./../000_ambient_loop_prototype_building_by_location").select{ |e| File.directory? e }.each do |f|
          runner.registerInfo("Removing #{f} directory.")
          FileUtils.rm_f Dir.glob("#{f}/*")
          FileUtils.remove_dir(f,true)
        end
       ##AA added the chunk below to get rid of prototype measure directories 12/20 
         Dir.glob("./../001_ambient_loop_add_ets_system").select{ |e| File.directory? e }.each do |f|
          runner.registerInfo("Removing #{f} directory.")
          FileUtils.rm_f Dir.glob("#{f}/*")
          FileUtils.remove_dir(f,true)
        end
     
         ##AA added the chunk below to address naming of directories, 12/20. using the exact same file path as before didn't work 
         #Dir.glob("./**/SR1").select{ |e| File.directory? e }.each do |f|
         Dir.glob("./../**/SR1").select{ |e| File.directory? e }.each do |f|
          runner.registerInfo("Removing #{f} directory.")
          FileUtils.rm_f Dir.glob("#{f}/*")
          FileUtils.remove_dir(f,true)
